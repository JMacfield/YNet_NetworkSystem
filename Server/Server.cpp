...
bool Server::Initialize(IPEndpoint ip) {
	...
	if (Network::Initialize()) {
		...
		if (listeningSocket.Create() == YResult::Y_Success) {
			std::cout << "Socket successfully created." << std::endl;
			
			if (listeningSocket.Listen(ip) == YResult::Y_Success) {
...
#pragma region listener
		...
		if (listeningSocketFD.revents & POLLRDNORM) {
			...
			if (listeningSocket.Accept(newConnectionSocket, &newConnectionEndpoint) == YResult::Y_Success) {
...
#pragma endregion Code specific to the listening socket
		...
		for (int i = 1; i < use_fd.size(); i++) {
			...
			if (use_fd[i].revents & POLLRDNORM) {
				char buffer[YNet::g_MaxPacketSize];
				int bytesReceived = 0;
				bytesReceived = recv(use_fd[i].fd, buffer, YNet::g_MaxPacketSize, 0);
